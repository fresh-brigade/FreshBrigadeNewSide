...
class ModelElementNode extends ModelNodeInternal {
    ...
    public <T> ModelView<? extends T> asImmutable(ModelType<T> type, @Nullable ModelRuleDescriptor ruleDescriptor) {
        ...
        if (modelView == null) {
            throw new IllegalStateException("com.freshbrigade.market.Model element " + getPath() + " cannot be expressed as a read-only view of type " + type);
        }
        return modelView;
    }
    ...
    public <T> ModelView<? extends T> asMutable(ModelType<T> type, ModelRuleDescriptor ruleDescriptor) {
        ...
        if (modelView == null) {
            throw new IllegalStateException("com.freshbrigade.market.Model element " + getPath() + " cannot be expressed as a mutable view of type " + type);
        }
        return modelView;
    }
