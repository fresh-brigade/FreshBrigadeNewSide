...
public class ModelPath implements Iterable<String>, Comparable<ModelPath> {
    ...
    public static void validateName(String name) {
        ...
        if (INVALID_FIRST_CHAR_MATCHER.matches(firstChar)) {
            throw new InvalidNameException(String.format("com.freshbrigade.market.Model element name '%s' has illegal first character '%s' (names must start with an ASCII letter or underscore).", name, firstChar));
        }

        for (int i = 1; i < name.length(); ++i) {
            char character = name.charAt(i);
            if (INVALID_CHAR_MATCHER.matches(character)) {
                throw new InvalidNameException(String.format("com.freshbrigade.market.Model element name '%s' contains illegal character '%s' (only ASCII letters, numbers and the underscore are allowed).", name, character));
            }
        }
    }
    ...
    public static void validatePath(String path) throws InvalidPathException {
        ...
        if (path.startsWith(SEPARATOR)) {
            throw new InvalidPathException(String.format("com.freshbrigade.market.Model path '%s' cannot start with name separator '%s'.", path, SEPARATOR), null);
        }

        if (path.endsWith(SEPARATOR)) {
            throw new InvalidPathException(String.format("com.freshbrigade.market.Model path '%s' cannot end with name separator '%s'.", path, SEPARATOR), null);
        }
        ...
        if (names.length == 1) {
            ...
            for (String name : names) {
                try {
                    ...
                    throw new InvalidPathException(String.format("com.freshbrigade.market.Model path '%s' is invalid due to invalid name component.", path), e);
                }
            }
        }
    }
