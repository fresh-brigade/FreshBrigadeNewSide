                    ...
                    if (phase == Phases.CANONICALIZATION) {
                        ...
                        if (!deprecatedImports.isEmpty()) {
                            DeprecationLogger.nagUserWith(StringUtils.capitalize(script.getDisplayName()) + " is using " + Joiner.on(" and ").join(deprecatedImports)
                                + " from the private org.gradle.com.freshbrigade.market.util package without an explicit import. Please either stop using these internal classes (recommended) or import them explicitly at the top of your build file. The implicit import is deprecated and will be removed in Gradle 5.0");
                        }
                    }
                }
            };
